...
// Draws the ARCore camera-passthrough texture as a full-screen quad. No
// depth/occlusion handling, no plane/point-cloud rendering — passthrough
// only, per Milestone 1's scope. Shader/coordinate-transform approach
// follows Google's official hello_ar_kotlin sample, the best-documented
// reference for GLES code this environment can't build or run locally to
// iterate on.
internal class BackgroundRenderer {
    ...
    private val quadCoords = toFloatBuffer(NDC_QUAD_COORDS)
    private val texCoordsScratch = FloatArray(NDC_QUAD_COORDS.size)
    private var texCoords = toFloatBuffer(texCoordsScratch)
    ...
    // Recomputes texture coordinates whenever the display geometry changes
    // (rotation, foldable state, etc.) and draws the camera image.
    fun draw(frame: Frame) {
        if (frame.hasDisplayGeometryChanged()) {
            ...
            texCoords = toFloatBuffer(texCoordsScratch)
        }

        if (frame.timestamp == 0L) return // no camera image yet this session
    ...
    private companion object {
        val NDC_QUAD_COORDS = floatArrayOf(-1f, -1f, +1f, -1f, -1f, +1f, +1f, +1f)
